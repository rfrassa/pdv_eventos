"""
load_test.py - Prueba de carga para pagina estatica en Netlify CDN
...
def fetch(url):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=TIMEOUT_SECONDS) as resp:
            ...
            return {"status": resp.status, "ms": elapsed_ms, "bytes": len(body), "error": None}
    except Exception as exc:
        elapsed_ms = (time.perf_counter() - start) * 1000
        return {"status": 0, "ms": elapsed_ms, "bytes": 0, "error": str(exc)}


def run_stage(concurrent_users):
    print()
    print("=" * 60)
    print("  Etapa: " + str(concurrent_users) + " usuarios concurrentes")
    ...
    print("  Etapa completada en " + f"{stage_elapsed:.2f}" + "s", flush=True)
    return results, stage_elapsed


def print_summary(results, stage_elapsed, concurrent_users):
    total = len(results)
    ...
    print()
    print(f"  {'Metrica':<25} {'Valor':>12}")
    print(f"  {'-'*38}")
    print(f"  {'Total peticiones':<25} {total:>12}")
    print(f"  {'Exitosas (HTTP 200)':<25} {len(successes):>12}")
    print(f"  {'Fallidas':<25} {len(failures):>12}")
    print(f"  {'Tasa de exito':<25} {success_rate:>11.2f}%")
    ...
    print(f"  {'Tiempo maximo':<25} {max_ms:>10.1f} ms")
    print(f"  {'Throughput':<25} {throughput:>9.1f} req/s")
    print(f"  {'Tamano promedio resp.':<25} {avg_bytes:>9.0f} B")
...
def print_final_verdict(stage_summaries):
    print()
    print("=" * 60)
    print("  VEREDICTO FINAL")
    print("=" * 60)
    print()
    print(f"  {'Etapa':<10} {'Exito %':>8} {'p95 ms':>9} {'Req/s':>8}  Resultado")
    print(f"  {'-'*55}")

    all_pass = True
    for s in stage_summaries:
        ok = s["p95_ms"] < 2000 and s["success_rate"] > 99.0
        if not ok:
            all_pass = False
        estado = "OK" if ok else "FALLO"
        print(f"  {s['concurrent_users']:<10} {s['success_rate']:>7.2f}% {s['p95_ms']:>8.1f} {s['throughput']:>8.1f}  [{estado}]")

    print()
    if all_pass:
        print("  >> CDN aguanta: p95 < 2000ms y exito > 99% en todas las etapas.")
    else:
        print("  >> revisar: alguna etapa supero p95 >= 2000ms o exito <= 99%.")
    print()
...
def main():
    ...
    print("=" * 60)
    print("  Objetivo: " + TARGET_URL)
    print("  Etapas:   " + str(STAGES) + " usuarios concurrentes")
    print("  Timeout:  " + str(TIMEOUT_SECONDS) + "s por peticion")
